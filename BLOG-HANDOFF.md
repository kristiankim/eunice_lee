...
3. Set `VITE_FORM_ENDPOINT` in the host to the secure form endpoint that delivers inquiries to Eunice’s confirmed email. The current local build uses a preview-only success state when this variable is absent.
...
1. Replace the placeholder in `src/App.jsx`’s `Blog` component with approved posts or connect the route to the chosen CMS.
